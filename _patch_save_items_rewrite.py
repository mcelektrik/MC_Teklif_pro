...
# locate the "Update Items" section inside save_draft
start = None
end = None
for i,ln in enumerate(src):
    if "Update Items" in ln and "self.current_offer" in "".join(src[max(0,i-8):i+1]):
        start = i
        break
if start is None:
    raise SystemExit("Could not find 'Update Items' section")

# end at calculate_offer_totals(self.current_offer) line (exclusive)
for j in range(start, len(src)):
    if "calculate_offer_totals(self.current_offer)" in src[j]:
        end = j
        break
if end is None:
    raise SystemExit("Could not find end anchor: calculate_offer_totals(self.current_offer)")

indent = src[start].split("#")[0]  # same indentation as section comment

# Build safe replacement block (offer_id-based, no relationship append, clears identity map)
block = []
...
block.append(f"{indent}# delete old items in DB\n")
...
block.append(f"{indent}# clear identity map noise for OfferItem\n")
